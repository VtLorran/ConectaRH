...
export default function CentralFerias() {
  ...
  const navigation = [
    ...
    { label: "Central de Férias", href: "/ferias" }
  ];
  ...
  const fetchVacations = async () => {
    try {
      ...
      if (json.success && json.data) {
        setVacations(json.data);
      }
    } catch (err) {
      console.error("Erro ao buscar histórico de férias:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRequestVacation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("Solicitação de férias enviada com sucesso!");
      setStartDate("");
      setEndDate("");
      setComment("");
  ...
  return (
    <SectionComponent>
      <TittleHeader tittle="Central de Férias" className="w-full" />
      ...
      <div className="w-full bg-white rounded-3xl shadow-xl border border-stone-100 p-8 mt-6">
        ...
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center text-center gap-3">
            ...
            <p className="text-stone-400 text-sm font-medium">Carregando histórico de férias...</p>
          </div>
        ) : (
            ...
            {vacations.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {vacations.map((v) => {
                  ...
                  const statusInfo = getStatusDetails(v.status, v.endDate);
              ...
              <div className="py-16 flex flex-col items-center justify-center text-center gap-3 bg-stone-50/20 rounded-2xl border border-stone-150">
                <Palmtree size={44} className="text-stone-300 animate-pulse" />
                <p className="text-stone-600 font-bold text-sm">Nenhuma solicitação de férias registrada</p>
                <p className="text-stone-450 text-xs max-w-sm">
                  Precisa de um descanso? Clique no botão no canto superior direito para solicitar férias ao time de Recursos Humanos.
                </p>
              </div>
            )}
          </div>
      ...
      {isModalOpen && (
        <Modal
          ...
          maxWidth="max-w-md"
        >
          <form onSubmit={handleRequestVacation} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                disabled={submitting}
                placeholder="Informe o motivo da solicitação de férias..."
                required
